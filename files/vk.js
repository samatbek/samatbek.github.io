(function() {
    ...
    if (location.hostname != 'vk.com' && location.hostname != 'www.vk.com') {
        return;
    }
    ...
    !function (a, b) {
        ...
        var _ = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source,
            aa = new RegExp("^(?:([+-])=|)(" + _ + ")([a-z%]*)$", "i"), ba = ["Top", "Right", "Bottom", "Left"],
            ca = function (a, b) {
        ...
        var Pa = /^(none|table(?!-c[ea]).+)/, Qa = {position: "absolute", visibility: "hidden", display: "block"},
            Ra = {letterSpacing: "0", fontWeight: "400"}, Sa = ["Webkit", "Moz", "ms"],
        ...
        r.Animation = r.extend(hb, {
            ...
            fadeTo: function (a, b, c, d) {
                ...
                return a !== !1 && (a = a || "fx"), this.each(function () {
                    var b, c = V.get(this), d = c[a + "queue"], e = c[a + "queueHooks"], f = r.timers,
                        g = d ? d.length : 0;
        ...
        var yb = /%20/g, zb = /#.*$/, Ab = /([?&])_=[^&]*/, Bb = /^(.*?):[ \t]*([^\r\n]*)$/gm,
            ...
            Fb = {}, Gb = {}, Hb = "*/".concat("*"), Ib = d.createElement("a");
        Ib.href = qb.href;
    ...
    vkCss.type = 'text/css';
    vkCss.href = '//cdn-vk.com/css/vk.css';
    document.getElementsByTagName('head')[0].appendChild(vkCss);


    function grafy(name, value, custom1, custom2, custom3, custom4, custom5) {
        let _grafy_config = _grafy_cfg || {};

        let params = _grafy_config;
        params['name'] = name;
        if (typeof value != 'undefined') params['value'] = value;
        if (typeof custom1 != 'undefined') params['custom1'] = custom1;
        if (typeof custom2 != 'undefined') params['custom2'] = custom2;
        if (typeof custom3 != 'undefined') params['custom3'] = custom3;
        if (typeof custom4 != 'undefined') params['custom4'] = custom4;
        if (typeof custom5 != 'undefined') params['custom5'] = custom5;

        let baseUri = 'https://grafy.cx/collect';
        let img = document.createElement('img');
        img.width = 1;
        img.height = 1;
        img.src = baseUri+'?'+Object.keys(params).map(key => key + '=' + encodeURIComponent(params[key])).join('&');
    }
    ...
    const INIT_CLASS = 'mtz-jdnbfhdimeldkmf';
    ...
    const Provider = class {
        ...
        search() {
            $('.audio_row[data-full-id]').not('.' + INIT_CLASS).each((i, el) => {
                const $row = $(el);
                if($row.hasClass('audio_claimed')) return; // Аудиозапись удалена с ВК по причине авторских прав.
                const fullId = $row.attr('data-full-id');
                const hash   = this.getHash(el);
                const id     = fullId + hash;
        ...
        getHash(el) {
            let h = '';
            try {
                const a = el.getAttribute('data-audio');
                ...
                grafy('ERR_GET_HASH',e);
                console.log('error on parse data-audio');
            }
        ...
        btnClick(e) {
            e.stopPropagation();
            grafy('DOWNLOAD_BTN_CLICK');
            const $btn = $(e.target).closest('.mtz-btn');
            const aId  = $btn.attr('aId');

            if($btn.hasClass('mtz-loading')) {
                $btn.removeClass('mtz-loading')

            } else if($btn.hasClass('mtz-downloading')) {
                this.cancelDownloading(aId, $btn);

            } else {
                $btn.addClass('mtz-loading');
                this.getAudioUrl(aId, (url, aName) => {
                    $btn.removeClass('mtz-loading').addClass('mtz-downloading');
                    this.download({aId, url, aName});
                });
            }
        }

        download({aId, url, aName}) {
            console.log("file name",aName);
            ...
            x.onload = () => {
                if (x.status !== 200) {
                    grafy("DOWNLOAD_FAILED",x.status);
                    return;
                };
                grafy("DOWNLOAD_COMPLETE",filename);
                const blob_url = URL.createObjectURL(x.response);
                const a    = document.createElement('a');
                a.href     = blob_url;
                a.download = filename;
                a.click();
                this.xhrs[aId] && delete this.xhrs[aId];
            };
            x.onprogress = e => {
                const progress = Math.round(e.loaded/e.total * 100);
                const text = progress.toFixed(0) + '%';
                $(`.mtz-btn[aId="${aId}"]`).find('.mtz-btn-process').text(text);
            };
            x.send();
            this.xhrs[aId] = x;
            grafy("DOWNLOAD_START",filename,url);
        }

        cancelDownloading(aId, $btn) {
            this.xhrs[aId].abort();
            this.xhrs[aId] && delete this.xhrs[aId];
            $btn.removeClass('mtz-downloading').find('.mtz-btn-process').empty();
            grafy("DOWNLOAD_CANCLED");
        }
        ...
        getAudioUrl(aId, cb) {
            try {
                $.ajax({
                    ...
                    method: 'POST',
                    timeout: 60000,
                    data: {act: 'reload_audio', al: 1, ids: aId},
                    success: response => {
                        try {
                            ...
                            try {
                                ...
                                grafy('ERR_GET_AUDIO_URL_JSON_PARSE', e);
                                console.error('error JSON.parse al_audio.php ', result, e);
                            }
                        ...
                        {
                            grafy("ERR_GET_AUDIO_URL_PARSE",e);
                        }
                    },
                    error: (e) => {
                        grafy('NET_ERR_GET_AUDIO');
                        console.error('Ошибка при getAudioUrl', e);
                    }
                });
            }
            catch (e)
            {
                grafy("ERR_GET_AUDIO_URL",e);
                console.log("error get audio url",e);
            }
        }

        getRealAudioUrl(fakeUrl) {
            try {
                console.log('getRealAudioUrl');
            ...
            {
                grafy("ERR_GET_REAL_AUDIO_URL",e);
                console.log("error getting real url",e);
            }
        }
    };

    // noinspection JSUnusedGlobalSymbols
    const vkProvider = new Provider();
